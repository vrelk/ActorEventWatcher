...
Keyword Property ArgProxy Auto
Keyword Property KhaProxy Auto
	...
	If VampTarget.HasKeyword(ImATurnedVampire) == True	
		...
		trace("VampTarget == " + VampTarget + ", is this right?")
			...
			If VampireVictimAppearance.GetValue() == 0
				; RaceCompatibility compatibility
				If (ArgProxy == None)
					ArgProxy = Game.GetFormFromFile(0x00001D8C, "RaceCompatibility.esm") As Keyword
				EndIf
				If (KhaProxy == None)
					KhaProxy = Game.GetFormFromFile(0x00001D8B, "RaceCompatibility.esm") As Keyword
				EndIf
			
				Race victRace = VampTarget.GetActorBase().GetRace()		
		
				If (victRace == ArgonianRace || victRace.HasKeyword(ArgProxy))
					;VampTarget.SetEyeTexture(BVSkinEyesMaleArgonianVampire)
					VampTarget.SetEyeTexture(SkinEyesMaleArgonianVampire)
				ElseIf (victRace == KhajiitRace || victRace.HasKeyword(KhaProxy))
				...
				EndIf	
				; end RaceCompatibility compatibility
			EndIf
			
			If VampireVictimAppearance.GetValue() == 20000
			
				; RaceCompatibility compatibility
				If (PlayableRaceList == None)
					PlayableRaceList = Game.GetFormFromFile(0x00000D62, "RaceCompatibility.esm") As FormList
				EndIf
				If (PlayableVampireList == None)
					PlayableVampireList = Game.GetFormFromFile(0x00000D63, "RaceCompatibility.esm") As FormList
				EndIf	
				bool found = false
				int index = 0
				Race targetActorRace = VampTarget.GetActorBase().GetRace()
				while(index < PlayableRaceList.GetSize()) && (!found)
					if(targetActorRace == (PlayableRaceList.GetAt(index) AS Race))
						VampTarget.SetRace(PlayableVampireList.GetAt(index) AS Race)
						found = True
					endif
					index += 1
				endwhile
...
Function SendTurnedVampFeedEvent(Actor akVampire, Actor akVictim, bool victimSleeping)
	...
	VrelkTools_Logging.Log("Attempting to send Turned Vampire Feeding event", "BVVampireFeedEvent", true)
	;ObjectReference obVictim = akVictim as ObjectReference
	If akVampire.HasKeyword(ImATurnedVampire) == True
		VrelkTools_Logging.Log("Sending Turned Vampire Feeding event", "BVVampireFeedEvent", true)
		int handle = ModEvent.Create("BetterVampires_TurnedVampireFeed")
    	If (handle)
			ModEvent.PushForm(handle, akVampire)
			ModEvent.PushForm(handle, akVictim)
			ModEvent.PushString(handle, victimSleeping)
			...
			While SearchForVictim > 0
				...
				Race RandomFeedTargetRace = RandomFeedTarget.GetRace()
					...
					If (PlayableRaceList == None)
						PlayableRaceList = Game.GetFormFromFile(0x00000D62, "RaceCompatibility.esm") As FormList
					EndIf					
					Bool bValidTarget = (RandomFeedTargetRace == FoxRace) || PlayableRaceList.HasForm(RandomFeedTargetRace)
					If bValidTarget
					...
					If RandomFeedTarget.HasKeyword(ActorTypeNPC) && RandomFeedTarget.HasKeyword(ActorTypeUndead) == False && RandomFeedTarget.HasKeyword(Vampire) == False && RandomFeedTarget != VampTarget && RandomFeedTarget != Game.GetPlayer() && RandomFeedTarget.IsChild() == False && RandomFeedTarget.IsGhost() == False  && RandomFeedTarget.IsDead() == False
						...
						VrelkTools_Logging.Log("NPC Vampire has finished feeding, maybe? NPCIsFeeding: " + NPCIsFeeding, "BVVampireFeedEvent", true)
						If NPCIsFeeding == True
...
Event OnEffectFinish(Actor VampTargetActor, Actor Caster)
	...
	If VampTarget.HasKeyword(ImATurnedVampire) == False	
		...
		Race targetActorRace = VampTarget.GetActorBase().GetRace()
		while(index < PlayableRaceList.GetSize()) && (!found)
			if(targetActorRace == (PlayableVampireList.GetAt(index) AS Race))
				VampTarget.SetRace(PlayableRaceList.GetAt(index) AS Race)
				found = True
			endif
			index += 1
		endwhile
